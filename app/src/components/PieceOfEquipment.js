...
const PieceOfEquipment = () => {
	...
	return (
    	<div className="piece_of_equipment" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
		...
		<h1>Witaj w Urządzeniu!</h1>
			...
			{userType === 'commercial' && (
				<button onClick={goToAddTimeSlot} style={{ alignSelf: 'flex-end'}}>Dodaj okno czasowe</button>
			)}
			<table border="1" cellPadding="10">
				...
				{recipe.map((recipe, index) => (
					<tr key={index} onClick={() => goToTimeSlot()} style={{ cursor: 'pointer' }}>
