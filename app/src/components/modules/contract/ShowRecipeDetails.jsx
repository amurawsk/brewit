                                                ...
                                                <div className={styles.ingredients}>
                                                    ...
                                                    {step.ingredients.map((ingredient, idx) => (
                                                        <div key={idx} className={styles.detailBox}>
                                                            <p>Nazwa: {ingredient.name}</p>
                                                            <p>Ilość: {ingredient.amount}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                ...
                                {orders && orders.length > 0 ? (
                                    orders.map((order, index) => (
                                        <div key={index} className={styles.detailBox}>
                                            ...
                                            <p>Zleceniobiorca: {order.contract_brewery_name}</p>
                                            <p>Utworzone dnia: {new Date(order.created_at).toLocaleDateString()}</p>
                                            <p>Typ piwa: {order.beer_type}</p>
                                            <p>Ocena:{' '}
                                                {order.rate === null ? (
                                                    <span className={styles.emptyRate}>brak</span>
                                                ) : order.rate ? (
